...
class Workflow:
    ...
    async def handle_incoming_message(
        ...
        if corr_id is None or jwe_token is None:
            ...
            return
        try:
            description = await self.process_image(filepath, corr_id, jwe_token)
            ...
            reason = f"Image classification failed: {e}"
        except ValidationError as e:
            reason = f"Classification validation failed: {e}"
        except Exception as e:
            reason = f"Unexpected error in classification: {e}"
        logger.error(reason)
        await self.reject_message(
    ...
    async def process_image(self, filepath: str, corr_id: str, jwe_token: str) -> str:
        ...
        logger.debug(f"classifying image: {filepath}")
        description = classify_image(image_data)
        logger.info(f"classified image: {filepath}")
        return description
